...
import {
  ...
  logCorrelationError,
  emitCorrelationMetrics,
  addCorrelationToAWSClients,
  CorrelationContext,
...
export function observableResolver(
  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (event: AppSyncResolverEvent<any>, context: Context) {
      ...
      // 相関IDコンテキストの抽出・生成
      const correlationContext = extractOrGenerateCorrelationId(event, context);
      
      // 相関IDコンテキストをログとX-Rayに設定
      setCorrelationContext(correlationContext, logger, tracer);
      
      // AWS SDKクライアントに相関ID情報を追加
      addCorrelationToAWSClients(correlationContext);
      ...
      try {
        ...
        // 成功メトリクス（相関ID付き）
        if (addMetrics) {
          emitCorrelationMetrics(correlationContext, metrics, `${resolverName}.Success`);
          emitCorrelationMetrics(correlationContext, metrics, `${resolverName}.Duration`, duration, 'Milliseconds');
        ...
        // エラーメトリクス（相関ID付き）
        if (addMetrics) {
          emitCorrelationMetrics(correlationContext, metrics, `${resolverName}.Error`);
          emitCorrelationMetrics(correlationContext, metrics, `${resolverName}.Duration`, duration, 'Milliseconds');
        }
  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (records: any[], context: Context) {
      ...
      try {
        ...
        for (const chunk of chunks) {
          ...
          while (retryCount <= maxRetries) {
            try {
              ...
              metrics.addMetric(
                `${processorName}.RecordsProcessed`,
                MetricUnit.Count,
                chunk.length
              );
              metrics.addMetric(
                `${processorName}.ChunkDuration`,
        ...
        metrics.addMetric(
          `${processorName}.SuccessRate`,
          MetricUnit.Percent,
          (processedCount / records.length) * 100
        );
